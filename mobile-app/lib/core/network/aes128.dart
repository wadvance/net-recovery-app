import 'dart:typed_data';

/// AES-128 puro en Dart (FIPS-197), suficiente para resolver el reto
/// anti-bot __test (AES-128-CBC, sin padding) de freehosting/iFastNet.
class Aes128 {
  static const List<int> _sBox = <int>[
    ,
  ];

  static const List<int> _invSBox = <int>[
    ,
  ];

  static const List<int> _rCon = <int>[0x01,0x02,0x04,0x08,0x10,0x20,0x40,0x80,0x1b,0x36];

  static Uint8List _keyExpansion(Uint8List key) {
    final w = List<int>.filled(176, 0);
    for (var i = 0; i < 16; i++) w[i] = key[i];
    var Nk = 4, Nr = 10;
    for (var i = Nk; i < 4 * (Nr + 1); i++) {
      var temp = Uint8List(4);
      temp[0] = w[4 * (i - 1) + 0];
      temp[1] = w[4 * (i - 1) + 1];
      temp[2] = w[4 * (i - 1) + 2];
      temp[3] = w[4 * (i - 1) + 3];
      if (i % Nk == 0) {
        // RotWord + SubWord
        final t = temp[0];
        temp = Uint8List(4);
        temp[0] = _sBox[w[4 * (i - 1) + 1]];
        temp[1] = _sBox[w[4 * (i - 1) + 2]];
        temp[2] = _sBox[w[4 * (i - 1) + 3]];
        temp[3] = _sBox[t];
        temp[0] ^= _rCon[i ~/ Nk - 1];
      }
      for (var j = 0; j < 4; j++) {
        w[4 * i + j] = w[4 * (i - Nk) + j] ^ temp[j];
      }
    }
    return Uint8List.fromList(w);
  }

  // state column-major like AES; block is 16 bytes.
  static void _addRoundKey(List<List<int>> s, Uint8List w, int rnd) {
    for (var c = 0; c < 4; c++) {
      for (var r = 0; r < 4; r++) {
        s[r][c] ^= (w[rnd * 16 + c * 4 + r]);
      }
    }
  }

  static void _invSubBytes(List<List<int>> s) {
    for (var r = 0; r < 4; r++) {
      for (var c = 0; c < 4; c++) s[r][c] = _invSBox[s[r][c]];
    }
  }

  static void _invShiftRows(List<List<int>> s) {
    for (var r = 1; r < 4; r++) {
      final row = List<int>.from(s[r]);
      for (var c = 0; c < 4; c++) s[r][c] = row[(c - r + 4) % 4];
    }
  }

  static int _xtime(int a) {
    a <<= 1;
    if (a & 0x100 != 0) a ^= 0x11b;
    return a & 0xff;
  }

  static int _mul(int x, int y) {
    var z = 0;
    for (var i = 0; i < 8; i++) {
      if ((y & 1) != 0) z ^= x;
      y >>= 1;
      x = _xtime(x);
    }
    return z;
  }

  static void _invMixColumns(List<List<int>> s) {
    for (var c = 0; c < 4; c++) {
      final a0 = s[0][c], a1 = s[1][c], a2 = s[2][c], a3 = s[3][c];
      s[0][c] = _mul(a0, 14) ^ _mul(a1, 11) ^ _mul(a2, 13) ^ _mul(a3, 9);
      s[1][c] = _mul(a0, 9) ^ _mul(a1, 14) ^ _mul(a2, 11) ^ _mul(a3, 13);
      s[2][c] = _mul(a0, 13) ^ _mul(a1, 9) ^ _mul(a2, 14) ^ _mul(a3, 11);
      s[3][c] = _mul(a0, 11) ^ _mul(a1, 13) ^ _mul(a2, 9) ^ _mul(a3, 14);
    }
  }

  static Uint8List decryptBlock(Uint8List key, Uint8List block) {
    final w = _keyExpansion(key);
    const Nr = 10;
    final state = List.generate(4, (_) => List<int>.filled(4, 0));
    for (var c = 0; c < 4; c++) {
      for (var r = 0; r < 4; r++) state[r][c] = block[c * 4 + r];
    }
    _addRoundKey(state, w, Nr);
    for (var rnd = Nr - 1; rnd >= 1; rnd--) {
      _invShiftRows(state);
      _invSubBytes(state);
      _addRoundKey(state, w, rnd);
      _invMixColumns(state);
    }
    _invShiftRows(state);
    _invSubBytes(state);
    _addRoundKey(state, w, 0);
    final out = Uint8List(16);
    for (var c = 0; c < 4; c++) {
      for (var r = 0; r < 4; r++) out[c * 4 + r] = state[r][c];
    }
    return out;
  }

  /// CBC decrypt sin padding (coincide con slowAES.decrypt del reto, donde
  /// el unpad es un no-op porque el plaintext no lleva PKCS7 valido).
  static Uint8List cbcDecrypt(Uint8List key, Uint8List iv, Uint8List ct) {
    final out = Uint8List(ct.length);
    var prev = Uint8List.fromList(iv);
    for (var off = 0; off < ct.length; off += 16) {
      final block = ct.sublist(off, off + 16);
      final dec = decryptBlock(key, block);
      for (var i = 0; i < 16; i++) out[off + i] = dec[i] ^ prev[i];
      prev = Uint8List.fromList(block);
    }
    return out;
  }
}
